...
const profile = buildProfile(profileArg?.slice("--profile=".length) || "standard");
const foliate = foliateElements();
...
const projectLicense = fs.readFileSync("LICENSE", "utf8");
const upstreamLicense = fs.readFileSync("licenses/elton-reader-MIT.txt", "utf8");
const fontLicense = fs.readFileSync(path.join("fonts", "OFL.txt"), "utf8");
const fontProvenance = fs.readFileSync(path.join("fonts", "README.md"), "utf8");
const banner = `/*
THIS IS A GENERATED/BUNDLED FILE BY ESBUILD
Build channel: ${profile.name}
If you want to view the source, please visit the plugin's GitHub repository:
https://github.com/newcoder/uvreader
...
const workerCode = (await esbuild.transform(loadPatchedWorker(), {
  target: "es2018", charset: "utf8", minifyWhitespace: prod, minifySyntax: prod,
...
const ctx = await esbuild.context({
  ...
  treeShaking: true,
  outfile: path.join(profile.outputDir, "main.js"),
  metafile: true,
  banner: { js: banner },
  plugins: [...profile.plugins, neutralise, foliate.plugin],
  charset: "utf8",
...
if (prod) {
  ...
  if (profile.name === "community") {
    ...
    fs.writeFileSync(path.join(profile.outputDir, "build-info.json"), JSON.stringify({
      profile: profile.name, inputs: Object.keys(result.metafile.inputs).sort(),
    }, null, 2) + "\n");
  }
  ...
  console.log(`build complete (${profile.name}) — pdf.worker embedded into main.js`);
  process.exit(0);
} else {
  await ctx.watch();
}
